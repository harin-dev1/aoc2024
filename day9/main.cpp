...
uint64_t get_file_compactness(uint32_t file_id, uint32_t file_size_in_blocks, uint32_t blk_offset) {
    ...
    while (file_size_in_blocks > 0) {
        //std::cout << blk_offset << "*" << file_id << std::endl;
...
uint64_t get_disk_compactness_a(std::string disk_map) {
    ...
    uint32_t j = disk_map.size() - 1;
    uint32_t blk_offset = 0;
    uint32_t free_space_in_blocks = 0;
    uint32_t ending_file_size_in_blocks = 0;
    while (i < j) {
        if (i % 2 == 0) {
            ...
            if (ending_file_size_in_blocks == 0) {
                ending_file_size_in_blocks = static_cast<uint32_t>(disk_map[j]) - '0';
            }
            if (free_space_in_blocks == 0) {
                free_space_in_blocks = static_cast<uint32_t>(disk_map[i]) - '0';
            }
            uint32_t file_id = j / 2;
            uint32_t num_blocks = std::min(ending_file_size_in_blocks, free_space_in_blocks);
            res += get_file_compactness(file_id, num_blocks, blk_offset);
            blk_offset += num_blocks;
            free_space_in_blocks -= num_blocks;
            ending_file_size_in_blocks -= num_blocks;
            if (ending_file_size_in_blocks == 0) {
                j -= 2;
            }
            if (free_space_in_blocks == 0) {
    ...
    if (ending_file_size_in_blocks != 0) {
        res += get_file_compactness(j / 2, ending_file_size_in_blocks, blk_offset);
    }
